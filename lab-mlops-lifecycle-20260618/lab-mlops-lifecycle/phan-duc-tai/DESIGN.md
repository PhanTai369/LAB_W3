...
**Concept drift (P(Y|X) thay đổi) không được detect trực tiếp** trong pipeline này nếu chỉ chạy chế độ data drift vì không có ground truth labels trong production. Tuy nhiên, bằng cách evaluate model precision/recall trên holdout dataset có nhãn, chúng ta có thể phát hiện cả concept drift.
